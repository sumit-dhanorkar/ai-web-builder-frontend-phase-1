...
export function ChatProvider({ children }: ChatProviderProps) {
  ...
  const { generateCategoryDescription } = useAutoGenerateDescription()
  const [sessionId, setSessionId] = useState<string | null>(null)

  // Initialize chat persistence hook (will be used once sessionId is set)
  const chatPersistence = useChatPersistence(sessionId)
   ...
   * Returns true if there's a sessionId in sessionStorage (indicates ongoing chat)
   */
  const hasMeaningfulChatData = (): boolean => {
    try {
      // Check if there's an existing session ID (user was in middle of chat)
      const sessionId = sessionStorage.getItem('current_session_id')
      if (sessionId) {
        console.log('✅ Found existing session in sessionStorage:', sessionId)
        return true
      }

      // Fallback: Check for old localStorage data (manual form)
  ...
  const initializeSession = useCallback(async () => {
    ...
    try {
      // Show global loader while initializing chat session
      showLoader('💬 Initializing chat session...')
      ...
      if (hasData && existingSessionId && sessionAge < SESSION_EXPIRY) {
        ...
        try {
          ...
          // Try to load messages from Firestore (will have widgets preserved)
          // Fall back to backend messages if Firestore load fails
          let messagesFromFirestore: Message[] = []
          try {
            console.log('📥 Loading messages from Firestore for session:', existingSessionId)
            messagesFromFirestore = await chatPersistence.loadMessages(existingSessionId)
            console.log(`✅ Loaded ${messagesFromFirestore.length} messages from Firestore`)
          } catch (error) {
            console.warn('Failed to load messages from Firestore, using backend messages:', error)
          }

          const messagesToUse = messagesFromFirestore.length > 0
          ...
          console.log(`✅ Session resumed with ${messagesToUse.length} messages`)
      ...
      if (!response) {
        ...
        // Send initial welcome message from AI and save to Firestore
        setTimeout(() => {
          sendWelcomeMessage(response.session_id)
        }, 500)
      }
      ...
      hideLoader() // Hide global page loader
    }
  }, [user?.uid, hideLoader])
   ...
   */
  const sendWelcomeMessage = useCallback(async (currentSessionId?: string) => {
    ...
    if (currentSessionId) {
      try {
        ...
        console.log('💾 Welcome message saved to Firestore')
      } catch (error) {
        console.warn('Failed to save welcome message to Firestore:', error)
      }
    }
  }, [apiClient])
    ...
    try {
      ...
      await chatAPI.streamMessage(
        ...
        (data: any) => {
          ...
          let finalData = data.updated_data
          ...
          // Auto-generate category description when transitioning to category_review
          if (data.next_state === 'category_review' && finalData?.category_name && !finalData?.category_description) {
            generateCategoryDescription(finalData.category_name, {
              category_name: finalData.category_name
            }).then((description) => {
              if (description) {
                const updatedData = { ...finalData, category_description: description }
                setCollectedData(updatedData)
                console.log('✓ Category description auto-generated')
              }
            }).catch((error) => {
              console.error('⚠️ Failed to auto-generate category description:', error)
            })
          }
          ...
          setMessages(prev =>
            prev.map(msg => {
              if (msg.id === userMessage.id) {
                const updated = { ...msg, status: 'sent' as const }
                // Save user message to Firestore
                chatPersistence.saveMessage(updated)
                return updated
              }
              if (msg.id === aiMessageId) {
                ...
                // Save AI message with widget to Firestore
                chatPersistence.saveMessage(updated)
                return updated
              }
              return msg
          ...
          if (data.next_state && sessionId) {
            chatPersistence.saveSession(
              data.next_state,
              data.progress?.section || 'welcome',
              data.updated_data || {}
            )
          }
          ...
          // Note: Messages and session are now saved to Firestore via chatPersistence hooks above
  ...
  }, [sessionId, currentState, collectedData, retryCount, chatPersistence])
