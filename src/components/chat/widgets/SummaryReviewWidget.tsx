...
export function SummaryReviewWidget({
  ...
  useEffect(() => {
    ...
    if (config.section === 'business_info') {
    ...
    } else if (config.section === 'certification' || config.section === 'certifications') {
      // Parse certifications if it's a JSON string
      let certifications = collectedData.certifications || []
      ...
      // Parse export_countries if it's a JSON string
      let exportCountries = collectedData.export_countries || []
      if (typeof exportCountries === 'string') {
        try {
          exportCountries = JSON.parse(exportCountries)
        } catch (e) {
          console.error('Failed to parse export countries:', e)
          exportCountries = []
        }
      }
      setFormData({
        export_countries: Array.isArray(exportCountries) ? exportCountries : []
      })
  ...
  // Watch for auto-generated category description and update formData when it arrives
  useEffect(() => {
    if (config.section === 'category' && collectedData.category_description && !formData.category_description) {
      console.log('✅ Category description auto-generated, updating form:', collectedData.category_description)
      setFormData((prev: any) => ({
        ...prev,
        category_description: collectedData.category_description
      }))
    }
  }, [collectedData.category_description, config.section])
  ...
  const handleSaveAndContinue = async () => {
    ...
    try {
      // Auto-generate category description in background
      if (config.section === 'category' && formData.category_name && !formData.category_description) {
        try {
          const generatedDescription = await generateCategoryDescription(formData.category_name, {
            category_name: formData.category_name
          })
          if (generatedDescription) {
            formData.category_description = generatedDescription
          }
        } catch (error) {
  ...
  const renderField = (
    ...
    else if (config.section === 'certification' || config.section === 'certifications') aiContextType = 'certification'
    ...
    <div className="mt-4 space-y-4">
      ...
      <div className="bg-gradient-to-br from-white to-teal-50/30 rounded-xl border-2 border-teal-200 shadow-lg p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          {(config.section === 'certification' || config.section === 'certifications') && <FileText className="w-5 h-5 text-teal-600" />}
        ...
        <div className="space-y-4">
            ...
            <>
              ...
              {formData.category_description ? (
                ...
                <p className="text-xs text-gray-500 italic">
                  Description will be auto-generated when you click "Save and Continue"
                </p>
              )}
            </>
          )}
          ...
          {(config.section === 'certification' || config.section === 'certifications') && (
            <div className="space-y-4">
              {/* Edit button */}
              <div className="flex items-center justify-between mb-2">
                <label className="text-sm font-medium text-gray-700">
                  Selected Certifications ({formData.certifications?.length || 0})
                </label>
                {!editingField || editingField !== 'certifications' ? (
                  <button
              ...
              {/* Editing Mode - Show CertificateSelectWidget */}
              {editingField === 'certifications' ? (
                <CertificateSelectWidget
                  ...
                  onComplete={(selectedCerts) => {
                    ...
                    handleChange('certifications', Array.isArray(certs) ? certs : [])
                    setEditingField(null)
                  }}
                  disabled={disabled}
                />
              ) : (
                /* Display Mode - Show selected certifications */
                <>
                  {formData.certifications && Array.isArray(formData.certifications) && formData.certifications.length > 0 ? (
                    <div className="space-y-3">
                      {formData.certifications.map((cert: any, idx: number) => (
                        <div key={idx} className="bg-gradient-to-br from-teal-50 to-emerald-50 rounded-lg border-2 border-teal-200 p-4">
                          <div className="flex items-start gap-3">
                            ...
                            {cert.certificate_url && (
                              <img
                                ...
                                alt={cert.name}
                                className="w-16 h-16 object-cover rounded-lg border border-teal-300 flex-shrink-0"
                              />
                            )}
                    ...
                    <div className="text-center py-8 text-gray-400">
                      ...
                      <p>No certifications selected</p>
                    </div>
                  )}
                </>
              )}
