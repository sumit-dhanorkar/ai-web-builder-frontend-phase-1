...
export function ProductStep({
  ...
  const { generateCategoryDescription, generateProductFields, isGenerating } = useAutoGenerateDescription()
  ...
  const openAddProductModal = async (categoryIndex: number) => {
    const category = categories[categoryIndex]

    // Generate category description if it's empty (only once per category)
    if (category && category.name.trim() && !category.description.trim()) {
      try {
        const description = await generateCategoryDescription(category.name, {
        ...
        if (description) {
          ...
          updatedCategories[categoryIndex].description = description
          onUpdateCategories(updatedCategories)
        }
      } catch (err) {
        console.error('Failed to generate category description:', err)
        // Continue anyway - don't block product modal from opening
      }
    }
  ...
  const saveProduct = async () => {
    ...
    // Generate product fields if they are empty (new product or missing fields)
    if (!isEditMode || !currentProduct.description.trim()) {
      const result = await generateProductFields(currentProduct.name, {
        category_name: categories[currentCategoryIndex]?.name || ''
      })

      if (result) {
        // Merge generated fields with current product
        productToSave = {
          ...productToSave,
          description: result.description || productToSave.description,
          specifications: {
            ...productToSave.specifications,
            ...result.specifications,
            origin: 'India' // Always set origin to India
          },
          key_benefits: result.key_benefits && result.key_benefits.length > 0 ? result.key_benefits : productToSave.key_benefits
        }
      }
    }

    // Save product with generated fields
    if (isEditMode && currentProductIndex !== null) {
        ...
        i === currentCategoryIndex
          ? {
              ...
              products: cat.products.map((prod, j) =>
                j === currentProductIndex ? productToSave : prod
              )
            }
          : cat
      )
      ...
      const updated = categories.map((cat, i) =>
        i === currentCategoryIndex
          ? { ...cat, products: [...cat.products, productToSave] }
          : cat
      )
      onUpdateCategories(updated)
    }
